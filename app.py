...
def update_medications(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        patient_medications = Medications_patient.query.filter_by(id=form_id).first()
        print('patient_medications', patient_medications)
        patient_medications.icd10_code = request.form.get('med_ndc')
        db.session.commit()
...
def delete_medication(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        print('form_id', form_mrn)
        print('form_med_ndc', form_med_ndc)
        patient_medication = Conditions_patient.query.filter_by(mrn=form_mrn, icd10_code=form_med_ndc).all()
        print('Found medications: ', patient_medication)
        for condition in patient_medication:
            db.session.delete(condition)
        db.session.commit()
